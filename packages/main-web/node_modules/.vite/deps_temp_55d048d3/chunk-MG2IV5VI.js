...
function checkProxyFunction(target, value) {
  if (isCallable(value) && !isBoundedFunction(value) && !isConstructable(value)) {
    if (!setFnCacheMap.has(target)) {
      setFnCacheMap.set(target, /* @__PURE__ */ new WeakMap());
...
function getJsLoader(_ref22) {
  var plugins = _ref22.plugins, replace = _ref22.replace;
  return function(code) {
    var src = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : "";
    var base = arguments.length > 2 ? arguments[2] : void 0;
    return compose(plugins.map(function(plugin) {
    ...
    return import_regenerator.default.wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          ...
          processedCssList = getExternalStyleSheets().map(function(_ref22) {
            var src = _ref22.src, ignore = _ref22.ignore, contentPromise = _ref22.contentPromise;
            return {
              src,
              ignore,
...
var idToSandboxCacheMap = function(_window$__WUJIE_INJEC) {
  if ((_window$__WUJIE_INJEC = window.__WUJIE_INJECT) !== null && _window$__WUJIE_INJEC !== void 0 && _window$__WUJIE_INJEC.idToSandboxMap) return window.__WUJIE_INJECT.idToSandboxMap;
  else {
    var cacheMap = window.__POWERED_BY_WUJIE__ ? window.__WUJIE.inject.idToSandboxMap : /* @__PURE__ */ new Map();
    window.__WUJIE_INJECT = _objectSpread2(_objectSpread2({}, window.__WUJIE_INJECT), {}, {
      idToSandboxMap: cacheMap
    });
    return cacheMap;
  }
}();
...
function addSandboxCacheWithWujie(id, sandbox) {
  ...
  if (wujieCache) idToSandboxCacheMap.set(id, _objectSpread2(_objectSpread2({}, wujieCache), {}, {
    wujie: sandbox
  }));
...
var documentProxyProperties = {
  // 降级场景下需要本地特殊处理的属性
  modifyLocalProperties: ["createElement", "createTextNode", "documentURI", "URL", "getElementsByTagName", "getElementById"],
  // 子应用需要手动修正的属性方法
...
var appWindowOnEvent = ["onload", "onbeforeunload", "onunload", "onerror", "onunhandledrejection"];
...
var windowRegWhiteList = [/animationFrame$/i, /resizeObserver$|mutationObserver$|intersectionObserver$/i, /height$|width$|left$/i, /^screen/i, /CSSStyleSheet$/i, /X$|Y$/];
...
var rawInsertAdjacentElement = HTMLStyleElement.prototype.insertAdjacentElement;
    ...
    insertAdjacentElement: {
      value: function value(position, element) {
        if (element.nodeName === "STYLE") {
          nextTick(function() {
            return handleStylesheetElementPatch(element, sandbox);
          });
          var res = rawInsertAdjacentElement.call(this, position, element);
          sandbox.styleSheetElements.push(element);
          return res;
      ...
      switch ((_element$tagName = element.tagName) === null || _element$tagName === void 0 ? void 0 : _element$tagName.toUpperCase()) {
        case "LINK": {
          ...
          if (href && !isMatchUrl(href, getEffectLoaders("cssExcludes", plugins))) {
            ...
            }], fetch, lifecycles.loadError).forEach(function(_ref22) {
              var src2 = _ref22.src, ignore = _ref22.ignore, contentPromise = _ref22.contentPromise;
        ...
        case "SCRIPT": {
          ...
          var _ref32 = element, src = _ref32.src, text = _ref32.text, _type = _ref32.type, crossOrigin = _ref32.crossOrigin;
      ...
      while (1) switch (_context.prev = _context.next) {
        case 0:
          ...
          return Promise.all([Promise.all(_getExternalStyleSheets(cssBeforeLoaders, sandbox.fetch, sandbox.lifecycles.loadError).map(function(_ref) {
            ...
            contentList.forEach(function(_ref22) {
              var src = _ref22.src, content = _ref22.content;
              if (!content) return;
          ...
          }), Promise.all(_getExternalStyleSheets(cssAfterLoaders, sandbox.fetch, sandbox.lifecycles.loadError).map(function(_ref32) {
            var src = _ref32.src, contentPromise = _ref32.contentPromise;
            return contentPromise.then(function(content) {
              return {
                src,
                content
...
function clearInactiveAppUrl() {
  ...
  var newQuery = "?" + Object.keys(queryMap).map(function(key) {
    return key + "=" + window.decodeURIComponent(queryMap[key]);
  }).join("&");
  winUrlElement.search = newQuery;
  if (winUrlElement.href !== window.location.href) {
    window.history.replaceState(null, "", winUrlElement.href);
...
function patchIframeHistory(iframeWindow, appHostPath, mainHostPath) {
  ...
  history.pushState = function(data, title, url) {
    ...
    rawHistoryPushState.call(history, data, title, ignoreFlag ? void 0 : mainUrl);
  ...
  history.replaceState = function(data, title, url) {
    ...
    var mainUrl = getAbsolutePath(url === null || url === void 0 ? void 0 : url.replace(appHostPath, ""), baseUrl);
    var ignoreFlag = url === void 0;
    rawHistoryReplaceState.call(history, data, title, ignoreFlag ? void 0 : mainUrl);
    if (ignoreFlag) return;
    updateBase(iframeWindow, appHostPath, mainHostPath);
    syncUrlToWindow(iframeWindow);
  };
  ...
  documentEvents.forEach(function(propKey) {
    ...
    try {
      Object.defineProperty(iframeWindow.Document.prototype, propKey, {
        ...
        set: descriptor.writable || descriptor.set ? function(handler) {
          (sandbox.degrade ? sandbox : window).document.removeEventListener(propKey, handlerCallbackMap.get(handler));
          (sandbox.degrade ? sandbox : window).document.addEventListener(propKey, typeof handler === "function" ? handler.bind(iframeWindow.document) : handler);
...
function stopIframeLoading(iframe, useObjectURL) {
  ...
  return new Promise(function(resolve) {
    function loop() {
      setTimeout(function() {
        ...
        if (useObjectURL) {
          var href = iframeWindow.location.href;
          newDoc.open();
          newDoc.close();
          var deadline = Date.now() + 1e3;
          var _loop = function loop2() {
            ...
            if (iframeWindow.location.href === href) setTimeout(_loop, 1);
            else resolve();
          };
          _loop();
          return;
...
var _ref2 = function() {
  var disabledMarkKey = "wujie:disableSandboxEmptyPageURL";
  var disabled = false;
  try {
    disabled = localStorage.getItem(disabledMarkKey) === "true";
  } catch (e) {
  }
  if (disabled || typeof URL === "undefined" || typeof URL.createObjectURL !== "function") return [function() {
    return "";
  }, function() {
    return void 0;
  }];
  var prevURL = "";
  var getSandboxEmptyPageURL2 = function getSandboxEmptyPageURL3() {
    if (disabled) return "";
    if (prevURL) return prevURL;
    var blob = new Blob(["<!DOCTYPE html><html><head></head><body></body></html>"], {
      type: "text/html"
    });
    prevURL = URL.createObjectURL(blob);
    return prevURL;
  };
  var disableSandboxEmptyPageURL2 = function disableSandboxEmptyPageURL3() {
    disabled = true;
    try {
  ...
  return [getSandboxEmptyPageURL2, disableSandboxEmptyPageURL2];
}();
var _ref3 = _slicedToArray(_ref2, 2);
var getSandboxEmptyPageURL = _ref3[0];
var disableSandboxEmptyPageURL = _ref3[1];
function iframeGenerator(sandbox, attrs, mainHostPath, appHostPath, appRoutePath) {
  ...
  sandbox.iframeReady = stopIframeLoading(iframe, useObjectURL && {
    mainHostPath
  }).then(function() {
...
function localGenerator(iframe, urlElement, mainHostPath, appHostPath) {
  ...
  Object.defineProperties(proxyDocument, {
    ...
    getElementById: {
      get: function get() {
        return function() {
          var id = arguments.length <= 0 ? void 0 : arguments[0];
          return sandbox.document.getElementById(id) || iframe.contentWindow.__WUJIE_RAW_DOCUMENT_HEAD__.querySelector("#".concat(id));
        };
      }
    }
  });
...
var appEventObjMap = function(_window$__WUJIE_INJEC) {
  if ((_window$__WUJIE_INJEC = window.__WUJIE_INJECT) !== null && _window$__WUJIE_INJEC !== void 0 && _window$__WUJIE_INJEC.appEventObjMap) return window.__WUJIE_INJECT.appEventObjMap;
  else {
    var cacheMap = window.__POWERED_BY_WUJIE__ ? window.__WUJIE.inject.appEventObjMap : /* @__PURE__ */ new Map();
    window.__WUJIE_INJECT = _objectSpread6(_objectSpread6({}, window.__WUJIE_INJECT), {}, {
...
var Wujie = function() {
  ...
  return _createClass(Wujie2, [{
              ...
              syncScriptResultList.concat(deferScriptResultList).forEach(function(scriptResult) {
                _this3.execQueue.push(function() {
                  return scriptResult.contentPromise.then(function(content) {
                    return _this3.fiber ? _this3.requestIdleCallback(function() {
                      return insertScriptToIframe(_objectSpread7(_objectSpread7({}, scriptResult), {}, {
                        content
                      }), iframeWindow);
                    }) : insertScriptToIframe(_objectSpread7(_objectSpread7({}, scriptResult), {}, {
                      content
                    }), iframeWindow);
                  });
                });
    ...
    key: "unmount",
    value: function() {
      var _unmount = _asyncToGenerator(import_regenerator3.default.mark(function _callee3() {
    ...
    key: "destroy",
    value: function() {
      var _destroy = _asyncToGenerator(import_regenerator3.default.mark(function _callee4() {
        var _this$iframe$parentNo, _iframeWindow;
...
function preloadApp(preOptions) {
  requestIdleCallback(function() {
    ...
    var options = mergeOptions(_objectSpread8({}, preOptions), cacheOptions);
    var name = options.name, url = options.url, html = options.html, props = options.props, alive = options.alive, replace = options.replace, fetch = options.fetch, exec = options.exec, attrs = options.attrs, degradeAttrs = options.degradeAttrs, fiber = options.fiber, degrade = options.degrade, prefix = options.prefix, plugins = options.plugins, lifecycles = options.lifecycles, iframeAddEventListeners = options.iframeAddEventListeners, iframeOnEvents = options.iframeOnEvents;
...
*/
//# sourceMappingURL=chunk-MG2IV5VI.js.map
